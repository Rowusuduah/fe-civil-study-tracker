...
- **Exam target**: configurable in Settings
...
- **Study plan generator** — phase-aware normal, intense, and final-sprint planning
- **Offline-capable PWA** — works without internet after first load
- **Optional Google Drive sync** — back up and restore across devices
- **Optional private exam baseline** — import diagnostic scores from your own JSON file
...
│   │   └── baseline.js         # Optional local baseline loader and validation
│   ├── engine/
...
### `js/data/baseline.js`
Loads an optional exam score profile from this browser's local storage. The public app ships no personal scores. Without a profile, baseline weakness is neutral and planning adapts to logged study activity. Import a profile through Settings → Private Exam Baseline; it is included in JSON backups and Google Drive sync.

The import file is JSON with a `subjects` object. Each entry uses a syllabus subject ID and a numeric score, tier, rank, and study weight. A generic example:
...
```json
{
  "subjects": {
    "mathematics": {
      ...
      "tier": "MID",
      "priority": 1,
      "studyWeightMultiplier": 1.2
    }
  }
}
```

If you previously used a built-in profile, import a privately saved copy once after updating. Existing logged sessions remain in this browser.
...
```js
{
  examDate: "2027-06-01", // example; set your own date
...
- **Intense phase** (user-configured start): 14h/day every day
...
baselineWeakness = 0.5 when no private profile is imported
```
